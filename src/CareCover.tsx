    ...
    <div style={{ position: "fixed", inset: 0, background: "#04141a", overflow: "hidden", fontFamily: UI, display: gone ? "none" : "block" }}>
      ...
      <div style={{ position: "absolute", inset: 0, display: "grid", placeItems: "center", textAlign: "center", opacity: opening ? 0 : 1, transition: "opacity .5s ease", pointerEvents: opening ? "none" : "auto", padding: 20 }}>
        <div>
          ...
          <div style={{ fontSize: 16, color: "#8fd4e2", marginTop: 8, maxWidth: 440, marginLeft: "auto", marginRight: "auto" }}>Coordination for care teams â€” cover, sorted before it becomes a crisis.</div>
          <p style={{ fontSize: 14.5, color: "#cdeef5", marginTop: 20, maxWidth: 520, marginLeft: "auto", marginRight: "auto", lineHeight: 1.6 }}>
            CareCover is built for care and nursing agencies, care homes, and any team that sends staff out to clients. When a carer calls off sick, it instantly finds qualified, available cover â€” matched by skill, area, and who's free â€” and reassigns the visit in one tap. New clients are matched to a local carer with spare capacity, existing clients stay with their regular carer, and training, leave and holidays are handled automatically. One live board keeps the whole team in step, so vulnerable people stay covered and coordinators stay sane.
          </p>
          <button onClick={open} style={{ marginTop: 30, border: "none", borderRadius: 12, padding: "15px 40px", fontSize: 16, fontWeight: 800, letterSpacing: 1, cursor: "pointer", fontFamily: UI, background: `linear-gradient(135deg,${BRAND},${BRANDDK})`, color: "#fff", boxShadow: "0 6px 24px rgba(14,116,144,.5)" }}>â–¶  ENTER</button>
        </div>
      </div>
    </div>
  );
